...
- **NumberOfGamesInParallel**: Number of games to run in parallel (console and WebGUI). Applies to Round Robin and Gauntlet; Cup, Swiss and Ladder always run sequentially. PreventMoveDeviation works with parallel play: a game that repeats an earlier game's opening and colours waits until that game has finished and merged its moves, so the parallelism you actually get is bounded by how many distinct opening/engine/colour combinations are ready - a Shared-distribution gauntlet, where the challenger has White in every game of an opening, degrades toward sequential. Replay from a reference PGN or a resumed PGN is unaffected (the file is complete before play starts), which is how the tuner uses it. In the WebGUI, starting a tournament with a value above 1 opens the multi-board grid (`/tournament-grid`); user adjudication of the running game is available when this is 1. The old name `NumberOfGamesInParallelConsoleOnly` is still accepted when loading.
